至此，定时任务执行结果的内容就告一段落吧。

下面，让我们来看看调度中心这一端过期日志是如何清除的。要想清除过期日志，首先得明确日志的过期时间，而日志的过期时间是要让用户自己配置的，就定义在配置文件中。请看下面代码块。
...
```
从上面的代码块中可以看出来，日志的过期时间默认为30天，当然，这个值用户是可以随意更改的。

## 引入新组件JobLogReportHelper清理过期日志
而清除过期日志也是需要引入新的组件的，该组件就是JobLogReportHelper。
```java
/**
...
这个组件的逻辑非常简单，可以分为两个功能逻辑：
- 统计调度中心所有被调度的定时任务的执行情况，返回饼状报表给前端展示
- 统计并清除过期的任务日志
