...
- xxljob-16：子任务的实现，支持多个任务间形成父子关系，通过界面配置串行触发

其中xxljob-01~xxljob-06，属于`xxljob`框架的核心脉络，前六节让你具备二次开发的能力

xxljob-07开始进入`rpc`章节，其实说到底，`xxl-job`的本质还是个`rpc`，无非是附加了定时任务的执行限制，远程调用的方法必须在固定时间调用。其他的失败重试，日志收集，路由策略，心跳检测等等，这些都是rpc的知识。
...
至此，我们已经手写完成了handson-spring, handson-tomcat, handson-xxljob

具备这些能力之后，相信大家足够胜任大多数的业务项目，甚至可以站在更高的角度审视系统，一览众山小。

我一直强调程序人应该拥有绝对的自信，因为个人能力是相对于薪水和年限的。就年限而言，十年经验 和 一年经验用十年 ，大不相同，而后者大有人在。
